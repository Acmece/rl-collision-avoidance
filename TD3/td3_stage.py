...
from model.td3 import transform_buffer
...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    for id in range(MAX_EPISODES):
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            a, logprob = select_action(env=env, state_list=state_list,
        ...
        if env.index == 0:
            ...
            if len(replay_memory) > BATCH_SIZE:
                td3_update_stage(policy=policy_list, optimizer=optimizer_list, batch_size=BATCH_SIZE, memory=replay_memory, epoch = step, 
                                        ...
                                        obs_size=OBS_SIZE, act_size=ACT_SIZE, polyak=polyak, policy_noise=policy_noise, noise_clip=noise_clip, policy_delay=policy_delay, max_action=MAX_ACTION, action_bound=action_bound)
                global_update += 1
